...
public interface Configuration extends Mutable, Accessible {
    ...
    String hideStrategy = "hideStrategy";

    @DefaultValue("moveOrClickOut")
    HideStrategy hideStrategy();

    enum HideStrategy {
        moveOrClickOut, clickOut, notHide
    }
    ...
    default void save() throws IOException {
        ...
        out.write("\n# < 窗口隐藏策略 >\n");
        out.write("# 可选值 moveOrClickOut:鼠标移出窗口一定范围隐藏, clickOut:鼠标在窗口外单击后隐藏, notHide:不隐藏\n");
        write.reAccept(pairOf(hideStrategy, getProperty(hideStrategy)));
        out.write("\n# < 翻译接口 >\n");
